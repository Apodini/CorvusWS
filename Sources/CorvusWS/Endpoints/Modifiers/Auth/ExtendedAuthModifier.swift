...
/// conforming to `ExtendedAuthEndpoint`. Requires an object `U` that represents the
/// user to authorize.
public class ExtendedAuthModifier<
...
extension ExtendedAuthEndpoint {
    ...
    public func auth<U: CorvusModelAuthenticatable>(
        _ user: ExtendedAuthModifier<Self, U>.UserKeyPath
    ) -> ExtendedAuthModifier<Self, U> {
        ExtendedAuthModifier(self, user: user)
    }
}
