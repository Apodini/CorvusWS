...
public final class ExtendedFilterModifier<
...
/// An extension that adds a `.filter()` modifier to `ExtendedReadEndpoints`.
extension ExtendedReadEndpoint {
    ...
    /// - Returns: An instance of an `ExtendedFilterModifier` with the supplied filter.
    public func filter(
        _ filter: ExtendedFilterModifier<Self>.Filter
    ) -> ExtendedFilterModifier<Self> {
        ExtendedFilterModifier(self, filter: filter)
    }
}
