...
/// conforming to `ExtendedReadEndpoint`.
public final class ExtendedChildrenModifier<
    R: ExtendedReadEndpoint,
    M: CorvusModel>:
...
/// An extension that adds a `.children()` modifier to `ExtendedReadEndpoints`.
extension ExtendedReadEndpoint {
    ...
    /// - Returns: An instance of an `ExtendedChildrenModifier` with the supplied
    /// `KeyPath` to the relationship.
    public func children<M: CorvusModel>(
        _ path: ExtendedChildrenModifier<Self, M>.ChildrenPath
    ) -> ExtendedChildrenModifier<Self, M> {
        ExtendedChildrenModifier(self, path: path)
    }
}
